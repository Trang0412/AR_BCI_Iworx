...
%% You can change me
SETTINGS_FILE_NAME = 'ECG_circulation_test.iwxset';
...
% Lets just get a power of 2 for the buffer size
DATA_BUFFER_SIZE =  2^(nextpow2(samplerate * num_analog_channels / max_grabs_per_second)); 
...
[b, a] = butter(5, [2 20]/(samplerate/2));
plot_wndow_time = 20; % in second
plot_wndow_pnts =  plot_wndow_time * samplerate;
...
while (toc(t)<TIME_SECONDS)
	...
	indices = 1 : num_samples_per_ch; % array of sample indices
	...
	for idx_ch = 1 : num_analog_channels
        ...
        % Pre-process nd plot data, TrangLT Feb 27, 2024
        filtered_data = filter(b, a, data(buffer_indices + idx_ch));
        for i = indices
            subplot(1,num_analog_channels,idx_ch);
            plot(filtered_data(i:i+plot_wndow_pnts), y(i:buffer_indices+plot_wndow_pnts), 'b-')
            xlim([i/samplerate (i+plot_wndow_pnts)/samplerate])
            pause(0.005)
        end
